...
以下のページに資材を用意しています。(git@github.com:rocketechgroup/dbt-container-for-bigquery.git から fork したリポジトリです)
...
dbt では、`/models/` 配下に sql ファイルを配置することで、自動でテーブルを作成し、管理することになっています。しかし今回、既存の環境にテーブルが存在するため、dbt ではテーブルを作成したくありません。
そこで、`/models/` 配下にダミーの sql ファイルを配置して、偽装します。
*フォルダ名は config で変更可能です。今回はデフォルトのフォルダ構成で実装しています。
