...
本記事では、データ基盤の運用課題に有効な手段の一つである dbt を紹介しつつ、実際にBigQuery 上のデータ基盤に導入する方法について解説します。既存の基盤に手を加えずに、以下に示すようなテーブルに対する自動テストや、メタデータを統一して公開するwebサービスを立ち上げることができます。
...
企業のデータ活用が盛んになって久しい昨今では、上記のような課題と対策は整理されてきています。有名なものとしてDMBOKがあるので、その一部をご紹介します[^note_2]。
...
# 部分的な導入によるメリット
データの品質チェックと、メタデータ管理機能を導入するだけでも、データ基盤の課題の多くに対応できます。また、dbt はデータ基盤のQCDを向上するのに非常に協力なつーるです。とはいえ、既存のデータ基盤を全て dbt に移行するのはコストもかかりますし、移行失敗によるリスクも大きいと思います。部分的な導入を行う一番のメリットは、一部の機能だけをクイックに導入して、移行しやすい環境を整えることあると思います。
