...
仮説検証のサイクルに合わせてレポートを作り直す必要があるのですが、その旅にSQLを書いてデータパイプラインを書いて…というのでは待ちくたびれてしまうでしょう。
以下の方針によって、短期間での改修を可能にします。

- tableau の処理は、複雑に作り込むと改修が難しくなります。表計算フィールドで表計算フィールドを参照させるようなものや、中間テーブルを作るLOD計算を多用するのは避けます。一方で、分析軸を切り替えたり、SUMをAVGに切り替えるような変更は非常に平易に行えるため、group by のような処理は積極的にtableauで行います。
- SQL はテストが難しく、高品質なクエリを作るにはどうしても時間がかかってしまいます。そこで、SQLで作成するテーブルは可能な限り減らし、様々な要望にも答えられるテーブルを用意します。
## 役割の分担
...
これらの各パートについて、以下に詳細を語ります。

## 分担詳細

以下のデータパイプラインを参考に記載します。
```mermaid
graph LR;
...
```
### カラム名の変更/単純変換/数値の計算
上図の `raw_table --> dimention_table` に該当する部分です。
...
### 特定の軸による集計
上図の `fact_table --> fact-dimention table` に該当する部分です。
...
集計の基本単位となる粒度の軸に揃えます。ECサイトであれば、コンバージョンログとして、商品、ユーザー、コンバージョン時刻で一意となるようなログをイメージしてください。
商品を購入する前に見た集客方法(=広告)が異なる場合、別の成果として計上したい場合もあると思います。その場合は、商品、ユーザー、集客経路、コンバージョン時刻で一意とする場合もあると思います。このような、分析の最小粒度となる情報を、非公式に fact table と呼ぶことにします。
データ量が多くなりやすいため、こちらも BigQuery 等のDWHで実行します。

### 集計軸同士によるテーブルの結合
上図の `属性つき問い合わせログ` を作成する部分です。
...
もし集計テーブルを作ってしまっていたら、group by の条件を地域から都道府県に書き換えて別のテーブルを作る必要があり、これは待っていられません。BIツールであれば容易に変更できる上、ソースとなるテーブルも同じものを使いまわせるので、クロス集計処理はBIツールで実施するのが無難だと思います。
(データが1億行オーダーの場合は、例えば週ごと、月ごとなど、分析粒度を見直すのが良いと考えます。)

### 可視化
最後に、線・棒グラフにしたり、傾向線を引いたりといったことは BI ツール側で行います。
ポイントとして、ありきたりな分析はBIツールで簡単に実施できることが多いので、無理にデータマートを作らないというのが大事です。
7日間移動平均、コホート図、フォールアウト図、といったものは、BIツールで作る方が計算ミスも起こしにくく簡単です。
一方で、これらの処理を SQL で再現するには、それなりのSQL経験も必要ですし、テスト工数も必要になると思います。
