...
# ドメインごとにパイプラインを作る、マイクロデータメッシュという発想
...
しかし、この層に分けるアプローチにも課題があります。
そこで、やりたい事ごとに都度パイプラインを作り、そこで生まれた汎用的なテーブルを再利用できるように公開するアプローチを考えました。

![ドメイン毎のパイプラインの図](/images/.png)
...
そこで今回は、5~10人程度の1チーム内において発生するパイプラインの管理の課題に着目し、明日からでも検討可能なアイデアとして記載しています。
...
この層を分けるアイデアは SSoT を起点としていて、複数の用途に利用するための (dbtで言う) mart 層の存在が共通点だと思います。
...
1.は想像しやすいのですが、一度汎用的なものとして作ったテーブルも、いずれ古くなります。
...
![daily_user_cv の パイプライン](/images/.png)
...
![daily_products_cv の パイプライン](/images/.png)

この時、 cv_events は毎日 0 時と 10 時に更新される状態となり、どうも歪です。10時に更新された cv_events は、0時~10時のデータが混ざっているため、前日までのデータをみたい daily_user_cv テーブルの参照元として使えなくなっています。cv_events は 汎化テーブルですが、0時に更新する場合と、10時に更新した場合とで、使える用途が異なっているわけです。
...
![monthly_user_cv の パイプライン](/images/.png)
...
![再利用した パイプライン](/images/.png)

しかし、そんな都合のいいことはなく、このやり方は、①テーブルが増えてくると既存のテーブルで使えそうなものを見つけられない、②見つかったとしてもあずかり知らぬところでテーブルの仕様が代わり、ある日突然動かなくなる と言うことが起きます。

これを「外部に公開するテーブルを明確にする」と言うアプローチで解決します。
「使っても良いテーブル」と言うのをある程度制限します。具体的には、各ドメインのパイプラインのうち、これはシンプルでいろんなものにも使えそうだ、と思ったものだけを、更新頻度・鮮度・スキーマ情報などのメタ情報をつけて、外部に公開します。
一度公開した外部テーブルは、ドメイン内部の開発事情では基本的に変更しません。どうしても変更する場合は、リネームやシャーディング等を用いてバージョンを管理します。

![外部公開テーブルのイメージ](/images/.png)

外部に公開したテーブルは、情報を標準化して、検索できるようにします。
マニュアルでやるならそれらのテーブルをスプレッドシートで管理したり、可能ならカタログのように横断で探せるようなメタ情報検索プラットフォームがあると良さそうです。

# 外部公開テーブルを運用していくためのルール

外部公開テーブルを使った、ドメイン毎のパイプラインの分解には、以下の制約が伴います
